...
import posthog import project_root
...
def main(args):
    ...
    with mlflow.start_run():
        ...
        # Required: these parameters are essential for model reproductibility
        ...
        #== STAGE1: Data loading and validation==
        print(" Loading data...")
        df=load_data(args.input)  # Load raw CSV data with error handling
        print(f"Data loaded: {df.shape(0)} rows, {df.shape(1)} columns")

        #=== CRITICAL: Data QUALITY VALIDATION===
        # This step is essential for production ML - validates data qquality before training
        ...
        if not is_valid:
            ...
            raise ValueError(f" Data quality chek failed. Issues: {failed}")
        else:
        ...
        #Save processed dataset for reproductibility and debugging
        ...
        # this ensures serving pipelines uses exact same features in exect same order
        import json, joblib
        ...
        #These artifacts ensures trainig and serving use identical transformation
        preprocessing_artifact= {
            "feature columns": feature_cols,  #Exact feature order
        ...
        # Calculate scale_pos_weight to hamdle imbalanced dataset 
        ...
        #IMPORTANT: These hyparameters were optimized through hyperparameter tuning
        ...
        model=XGBClassifier(
            ...
            colsample_bytree=0.98  #sample ration of features for each tree
        ...
        # Generate predictons and track inference time
        t1=time.time()

        proba=model.predict_proba(X_test)[:,1] # Get probability of churn (classe 1)
        ...
        mlflow.log_metric("pred_time", pred_time) # Track infrence performance
        ...
        recall=recall_score(y_test, y_pred)  #of actuals churners, how many did we catch?
        ...
        #===STAGE 7:Model Serialization and loggig===
        print(" Saving Model to MLflow... ")
        #Essential: log metric in MLflow's standard format for serving
        mlflow.sklearn.log_model(
            model,
            artifact_path="model" #This create a 'model/' folder in MLflow runartifacts
...
if ___name__ == " __main__":
    ...
    pd.add_argument("--target", type=str, default="Churn")
